...
import {
  ...
  Copy, Check, ExternalLink, Wifi, QrCode,
} from "lucide-react";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(201,165,90,0.08)", border: "1px solid rgba(201,165,90,0.25)", fontSize: 10, color: "#92712a" }}>
        ⚠️ Chỉ hoạt động trên <b>Safari</b>. Nếu đang dùng Chrome/Firefox trên iPhone, mở link bằng Safari.
      </div>
...
function DesktopGuide() {
  const steps: Step[] = [
    ...
    { icon: <CheckCircle2 size={13} />, text: 'Nhấn "Cài đặt" để xác nhận', sub: 'App sẽ mở như cửa sổ riêng, xuất hiện trong danh sách ứng dụng' },
  ];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(14,165,233,0.06)", border: "1px solid rgba(14,165,233,0.2)", fontSize: 10, color: "#0369a1" }}>
        💡 Nếu không thấy biểu tượng cài đặt: <b>Menu Chrome → Thêm công cụ → Tạo lối tắt</b>
      </div>
      {steps.map((s, i) => <StepCard key={i} num={i + 1} icon={s.icon} text={s.text} sub={s.sub} />)}
    </div>
  );
...
// ─── QR Code (SVG tự vẽ từ URL dùng Google Charts API thay thế) ──────────────
...
function QRBlock({ url }: { url: string }) {
  ...
  // Google Charts QR API — không cần thư viện
  const qrSrc = `https://chart.googleapis.com/chart?chs=180x180&cht=qr&chl=${encodeURIComponent(url)}&choe=UTF-8`;
  ...
  return (
    <div style={{ borderRadius: 14, border: "1px solid #bae6fd", background: "#fff", overflow: "hidden" }}>
      <div style={{ padding: "8px 16px", borderBottom: "1px solid #e0f2fe", background: "#f0f9ff" }}>
        <p style={{ fontSize: 8, fontWeight: 700, color: "#64748b", letterSpacing: "0.2em" }}>CHIA SẺ LINK CÀI ĐẶT</p>
      </div>
      <div style={{ padding: 16, display: "flex", gap: 16, alignItems: "center" }}>
        {/* QR */}
        <div style={{
          width: 90, height: 90, borderRadius: 10, border: "1px solid #e0f2fe",
          display: "flex", alignItems: "center", justifyContent: "center",
          background: "#f8fafc", flexShrink: 0, overflow: "hidden",
        }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={qrSrc} alt="QR" width={80} height={80} style={{ borderRadius: 6 }} />
        </div>
        {/* URL + copy */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 9, color: "#64748b", marginBottom: 4 }}>Quét QR hoặc gửi link này cho thiết bị khác:</p>
          <div style={{
            background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 8,
            padding: "6px 10px", display: "flex", alignItems: "center", gap: 8,
          }}>
            <p style={{ fontSize: 10, color: "#0c1a2e", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {url}
            </p>
            <button
              onClick={copy}
...
function NetworkInfo() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setAppUrl(window.location.origin + "/install");
    }
  }, []);

  return (
    <div style={{ borderRadius: 14, border: "1px solid #bae6fd", background: "#fff", overflow: "hidden" }}>
      <div style={{ padding: "8px 16px", borderBottom: "1px solid #e0f2fe", background: "#f0f9ff", display: "flex", alignItems: "center", gap: 6 }}>
        <Wifi size={10} style={{ color: "#0ea5e9" }} />
        <p style={{ fontSize: 8, fontWeight: 700, color: "#64748b", letterSpacing: "0.2em" }}>THÔNG TIN KẾT NỐI</p>
      </div>
      <div style={{ padding: "10px 16px" }}>
        <p style={{ fontSize: 10, color: "#64748b", marginBottom: 8, lineHeight: 1.5 }}>
          Đảm bảo thiết bị cần cài và máy chủ đang <b>chung mạng WiFi</b> (hoặc truy cập qua internet nếu đã có domain).
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {[
            { label: "Link cài app", value: appUrl || "Đang tải..." },
            { label: "Link đăng nhập", value: appUrl ? appUrl.replace("/install", "/login") : "..." },
          ].map(row => (
...
function FAQ() {
  ...
  const items = [
    { q: "App có chiếm bộ nhớ máy không?", a: "Rất ít. PWA chỉ lưu cache nhẹ (dưới 10MB). Không cần tải từ App Store hay Google Play." },
    { q: "Dữ liệu có bị mất khi xóa app không?", a: "Dữ liệu lưu trên server (SQLite). Khi cài lại, đăng nhập lại là có đầy đủ dữ liệu." },
    { q: "Có cần internet để dùng không?", a: "Cần kết nối tới server. Nếu dùng trong nội bộ (LAN), chỉ cần chung WiFi là đủ." },
...
export default function InstallPage() {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ width: "100%", maxWidth: 480, padding: "16px 20px 0", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        {installed && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div>
              ...
              <p style={{ fontSize: 9, color: "#10b981", marginTop: 2 }}>Đang chạy ở chế độ standalone.</p>
            </div>
          </motion.div>
        )}
            ...
            {PLATFORMS.map(p => (
              <button key={p.id}
                ...
                style={{
                  ...
                  border: `1.5px solid ${active === p.id ? "#0ea5e9" : "#e0f2fe"}`,
